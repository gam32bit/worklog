...
def select_multiple_from_recent(recent_items: list[str], prompt: str) -> list[str]:
    """
    Let user select or enter multiple items from recent list.
    ...
    for part in parts:
        if part.isdigit():
            idx = int(part) - 1
            if 0 <= idx < len(recent_items):
                result.append(recent_items[idx])
        elif part:
            result.append(part)
